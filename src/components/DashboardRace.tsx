            ...
            {myStats ? (
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        {/* Current Stats Recap (Mini) */}
                        <div className="lg:col-span-1 space-y-4">
                            <div className="card-glass p-5 rounded-2xl flex items-center gap-4 relative overflow-hidden border-l-4 border-indigo-500 h-[calc(50%-8px)]">
                                <div className="p-4 rounded-full bg-indigo-500/10 text-indigo-400">
                                    <Icons.Trophy className="w-8 h-8" />
                                </div>
                                <div>
                                    <div className="text-sm text-[color:var(--text-muted)] font-medium">Class Rank</div>
                                    <div className="text-3xl font-black">#{myStats.rank}</div>
                                    <div className="text-xs text-indigo-400">Top {Math.round((myStats.rank / myStats.totalStudents) * 100) < 20 ? '20%' : '50%'}</div>
                                </div>
                            </div>
                            <div className="card-glass p-5 rounded-2xl flex items-center gap-4 relative overflow-hidden border-l-4 border-purple-500 h-[calc(50%-8px)]">
                                <div className="p-4 rounded-full bg-purple-500/10 text-purple-400">
                                    <Icons.Users className="w-8 h-8" />
                                </div>
                                <div>
                                    <div className="text-sm text-[color:var(--text-muted)] font-medium">My Class</div>
                                    <div className="text-2xl font-black truncate">{myStats.className}</div>
                                    <div className="text-xs text-purple-400">{myStats.totalStudents} Students</div>
                                </div>
                            </div>
                        </div>
                    </div>
